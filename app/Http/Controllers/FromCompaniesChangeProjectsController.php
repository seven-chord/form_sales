...
class FromCompaniesChangeProjectsController extends Controller
{
    ...
    {
        $from_company__post_id = $request->from_company_id;
        $prefectures = Prefecture::all();
        $categories = Category::all();
        $pagenate_counts = 100;
        $fromCompany = FromCompany::where('id', $from_company__post_id)->first();
        $fromCompanies = FromCompany::all();

        $toCompanies = ToCompany::orderBy('id','asc')->where('possible_send_flag','0')->where('from_company_id',$from_company__post_id)->paginate($pagenate_counts);

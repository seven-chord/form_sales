...
<?php
    $from_company_post_id = $from_company_post_id;
    $json_from_company_post_id = json_encode( $from_company_post_id , JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
?>

<script>
    var  from_company_post_id =JSON.parse('<?php echo $json_from_company_post_id; ?>');
    console.log(from_company_post_id);
</script>
...
<div class="container">
    ...
    <div class="sale_start_container">
            ...
            <table class="table table-condensed from_company_container">
                <tr>
                  <th>営業担当者：{{ $fromCompany->send_charge }}</th>
                </tr>
                <tr>
                    <th>項目名（ショートカット）</th>
                    <th>項目値</th>
            ...
            <p>下の枠がグレーの場合、右のボタンを押して送信可能ページかを確認　　<span class="to_company_url_spare"></span></p>
    ...
    <div class="row justify-content-center">
        <div class="col-md-2">
            ...
            <form action="{{ url('/from_companies/from_company_project') }}" method="POST">
              {{ csrf_field() }}
                    <select class="form-control from-company" name="from_company_id" id="project-list">
                        @forelse ($fromCompanies as $company)
                            <option value="{{ $company->id }}" class="project-title" >{{ $company->project_name }}</option>
                        @empty
                            <option value="">-</option>
                        @endforelse
                    </select>
